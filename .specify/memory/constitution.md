...
## 문서 언어 정책 (Documentation Language Policy)

모든 speckit 문서(spec.md, plan.md, tasks.md, checklists 등)는 **한국어**로 작성한다.

- 모든 설명, 요구사항, 사용자 스토리, 성공 기준, 가정 등은 MUST 한국어로 작성한다.
- 기술 용어, 고유명사(컴포넌트명, 패키지명, 명령어 등), 한국어로 표현이 어색하거나 불명확한 개념은 영어를 사용한다.
  - 예: `@myorg/tokens`, `Storybook`, `WCAG 2.1 AA`, `TDD`, `FR-001` 등은 영어 유지
- 한국어 표현이 어렵거나 의미 전달이 불명확한 경우 영어를 사용한다 (영어 병기 허용).
- 코드, 명령어, 파일 경로는 영어로 작성한다.

**Rationale**: 팀의 주요 작업 언어가 한국어이므로 문서 가독성과 이해도를 높이기 위해 한국어를 기본 언어로 지정한다.
